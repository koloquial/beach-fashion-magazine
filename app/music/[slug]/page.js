...
const fetchArticleBySlug = async (slug) => {
  console.log('called');
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API}/music/${slug}`);
    if (!response.ok) {
  ...
  return (
    <div className='page-container'>
      ...
      <p className='small-text'>{article.date}</p>
        ...
        <img className='article-image-2' src={`${process.env.NEXT_PUBLIC_API}${article.images[1]}`} />
...
<img className='article-image-1' src={`${process.env.NEXT_PUBLIC_API}${article.images[0]}`} />
