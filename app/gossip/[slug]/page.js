...
const fetchArticleBySlug = async (slug) => {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API}/gossip/${slug}`);
    if (!response.ok) {
      ...
      <div>
        {article.content.map((section, index) => (
          <div key={index} style={{ marginBottom: '30px' }}>
            ...
            <img className={`article-image-${index % 2 === 0 ? '2' : '1'}`} src={`${process.env.NEXT_PUBLIC_API}${article.images[index]}`} />
            <p>{section.body}</p>
          </div>
        ))}
      </div>
